...
def delete_task(request, pk):
    ...
    task_to_delete = get_object_or_404(Tasks, pk=pk, user=request.user) # getting the task from the database
    task = delete_a_task(pk,user)

    if request.user == task_to_delete.user: # insure the the user is the owner of the the task
...
def edit_task(request, pk):
    task = get_object_or_404(Tasks, pk=pk, user=request.user) # Query the database to get the task to edit
